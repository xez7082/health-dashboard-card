/**
 * HEALTH DASHBOARD CARD – V79.5 (COMPLETE EDITION)
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  updateSensors() {
    ...
    // 1. POIDS ET BARRE DE PROGRESSION
    ...
    // 3. CAPTEURS DYNAMIQUES ET GRAPHIQUES (SPARKLINES)
    if (pData.sensors) {
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        .s-card { background: #111; padding: 10px; margin-bottom: 10px; border-left: 4px solid #38bdf8; position: relative; border-radius: 4px; }
        .del-btn { position: absolute; top: 5px; right: 5px; background: #f87171; border: none; color: white; padding: 2px 6px; cursor: pointer; border-radius: 3px; }
        .results-box { background: #333; border: 1px solid #38bdf8; max-height: 120px; overflow-y: auto; display: none; position: absolute; width: 90%; z-index: 1000; box-shadow: 0 5px 15px rgba(0,0,0,0.5); }
        .res-item { padding: 8px; cursor: pointer; font-size: 11px; border-bottom: 1px solid #444; }
        .res-item:hover { background: #38bdf8; color: black; }
      </style>
      <div class="ed-box">
        <div style="display:flex; gap:8px; margin-bottom:15px;">
            <button style="flex:1; padding:10px; background:${pKey==='person1'?'#38bdf8':'#444'}; border:none; color:white; font-weight:bold; cursor:pointer; border-radius:5px;" id="t-p1">PATRICK</button>
            <button style="flex:1; padding:10px; background:${pKey==='person2'?'#38bdf8':'#444'}; border:none; color:white; font-weight:bold; cursor:pointer; border-radius:5px;" id="t-p2">SANDRA</button>
        </div>
        ...
        <div class="section">
            <label>SENSORS (RECHERCHE AUTO)</label>
            ...
            ${(p.sensors || []).map((s, i) => `
              <div class="s-card">
                ...
                <input type="text" class="search-input" data-idx="${i}" placeholder="Tapez (ex: poids)..." value="${s.entity}">
                <div class="results-box" id="res-${i}"></div>
            ...
            <button style="width:100%; padding:10px; background:#4ade80; border:none; color:white; font-weight:bold; cursor:pointer; border-radius:4px;" id="add-sensor">➕ AJOUTER UN CAPTEUR</button>
        </div>
      </div>
    `;
    ...
    this.querySelectorAll('.search-input').forEach(input => {
        input.oninput = (e) => {
            ...
            if (filtered.length > 0) {
                ...
                resDiv.innerHTML = filtered.map(en => `<div class="res-item" data-en="${en}">${en}</div>`).join('');
                resDiv.querySelectorAll('.res-item').forEach(item => {
                    item.onclick = () => {
...
window.customCards.push({ type: "health-dashboard-card", name: "Health Dashboard V79.5" });

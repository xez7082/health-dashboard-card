 ...
 * AJOUTS : ÉPAISSEUR/COULEUR BORDURES + RÉGLAGE TAILLES TEXTES
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  static getStubConfig() {
    return {
      ...
      border_width: 1, border_color: "rgba(255,255,255,0.15)",
      ...
      person1: {
        ...
        imc_entity: "", imc_name: "IMC", imc_icon: "mdi:scale-bathroom", imc_x: 20, imc_y: 20, imc_w: 160, imc_h: 69, imc_circle: false, imc_size: 80,
        corp_entity: "", corp_name: "Corpulence", corp_icon: "mdi:human-male", corp_x: 20, corp_y: 35, corp_w: 160, corp_h: 69, corp_circle: false, corp_size: 80
      },
      person2: {
  ...
  _renderBlock(x, y, w, h, icon, name, valId, isCircle, circleSize, accent, round) {
      ...
      const fsVal = isCircle ? (finalH / 4) : (this._config.font_size_value || 14);
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .sensor-card { 
            ...
            border: ${this._config.border_width || 1}px solid ${this._config.border_color || 'rgba(255,255,255,0.15)'}; 
        ...
        .steps-gauge { position: absolute; top: 15px; right: 15px; width: 85px; height: 85px; z-index: 100; background: rgba(0,0,0,0.5); border-radius: 50%; display: flex; align-items: center; justify-content: center; border: ${this._config.border_width || 1}px solid ${this._config.border_color || 'rgba(255,255,255,0.15)'}; }
      ...
      <div class="main-container">
        ...
        <div class="rule-container"><div class="rule-track"><div class="rule-fill"></div><div id="progression-pointer" class="prog-pointer"><div id="pointer-label" class="pointer-bubble">--</div></div>
          <div class="mark" style="left:0%">DÉPART<br>${pData.start}kg</div><div id="mark-comfort" class="mark" style="left:50%;color:#fbbf24"><div class="mark-confort"></div>CONFORT<br>${pData.comfort}kg</div><div class="mark" style="left:100%">IDÉAL<br>${pData.ideal}kg</div>
        </div></div>
      </div>
    `;
...
class HealthDashboardCardEditor extends HTMLElement {
      ...
      <div class="ed-box">
        ...
        <div class="section">
            ${this._activeTab === 'design' ? `
                <div class="sub-sec">
                    <label>Bordure des Blocs</label>
                    <div class="grid">
                        <div><label>Épaisseur</label><input type="number" id="inp-bw" value="${this._config.border_width || 1}"></div>
                        <div><label>Couleur</label><input type="text" id="inp-bc" value="${this._config.border_color || 'rgba(255,255,255,0.15)'}"></div>
                    </div>
                </div>
                <div class="sub-sec">
                    <label>Tailles des Textes</label>
                    <div class="grid">
                        <div><label>Étiquettes (px)</label><input type="number" id="inp-fsl" value="${this._config.font_size_label || 10}"></div>
                        <div><label>Valeurs (px)</label><input type="number" id="inp-fsv" value="${this._config.font_size_value || 14}"></div>
                    </div>
                </div>
                <div class="grid">
            ...
            ${this._activeTab === 'health' ? `
                <div class="sub-sec">
                    ...
                    <div class="grid"><div class="chk"><input type="checkbox" id="chk-imcc" ${p.imc_circle?'checked':''}> <label style="margin:0">Rond</label></div><div><label>Taille</label><input type="number" id="inp-imcs" value="${p.imc_size || 80}"></div></div>
                </div>
                <div class="sub-sec">
                    <label>Corpulence (Entité)</label><input type="text" id="inp-corpe" value="${p.corp_entity || ''}">
                    <div class="grid"><div class="chk"><input type="checkbox" id="chk-corpc" ${p.corp_circle?'checked':''}> <label style="margin:0">Rond</label></div><div><label>Taille</label><input type="number" id="inp-corps" value="${p.corp_size || 80}"></div></div>
                </div>
            ` : ''}
            ${this._activeTab === 'sensors' ? `
                ...
                ${(p.sensors || []).map((s, i) => `
                  <div class="sub-sec">
                    ...
                    <div class="grid"><div class="chk"><input type="checkbox" class="s-circ" data-idx="${i}" ${s.is_circle?'checked':''}> <label style="margin:0">Rond</label></div><div><label>Taille</label><input type="number" class="s-size" data-idx="${i}" value="${s.circle_size || 80}"></div></div>
  ...
  _attachEvents(pKey) {
    ...
    if(this._activeTab === 'design') { 
        bind('#inp-bw', 'border_width', true); bind('#inp-bc', 'border_color', true);
        bind('#inp-fsl', 'font_size_label', true); bind('#inp-fsv', 'font_size_value', true);
        bind('#inp-ch', 'card_height', true); bind('#inp-is', 'icon_size', true);
    }

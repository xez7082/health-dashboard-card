/**
 * HEALTH DASHBOARD CARD – V3.5.1
 * FIX: ENREGISTREMENT ET PERSISTANCE DES DONNÉES
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    if (!this._config.person1) this._config.person1 = { sensors: [] };
    if (!this._config.person2) this._config.person2 = { sensors: [] };
    this.render();
  }
  ...
  render() {
    ...
    const getY = (cat) => { if(cat==='sommeil') return 350; if(cat==='sante') return 540; return 160; };
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .box { position: absolute; background: rgba(15, 23, 42, 0.7); display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 10; backdrop-filter: blur(12px); text-align: center; border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; }
        .ico { --mdc-icon-size: 26px; margin-bottom: 4px; }
        .lbl { font-size: 0.7em; opacity: 0.6; font-weight: bold; text-transform: uppercase; }
        .val { font-weight: 800; line-height: 1; }
        .uni { font-size: 0.6em; opacity: 0.5; }
        .weight-area { position: absolute; bottom: 35px; left: 5%; width: 90%; z-index: 20; background: rgba(255,255,255,0.03); padding: 25px; border-radius: 24px; border: 1px solid rgba(255,255,255,0.05); box-sizing: border-box;}
        .weight-track { position: relative; width: 100%; height: 10px; background: rgba(255,255,255,0.1); border-radius: 10px; margin-top: 50px; }
        .ptr { position: absolute; top: -10px; width: 6px; height: 30px; background: #fff; border-radius: 3px; transition: 1s; box-shadow: 0 0 15px #fff; }
      ...
      <div class="main">
        <div class="bg"></div>
        <div class="sw-btns"><button class="sw-btn ${cv==='person1'?'active':''}" id="btn-p1">PATRICK</button><button class="sw-btn ${cv==='person2'?'active':''}" id="btn-p2">SANDRA</button></div>
        ...
        ${(p.sensors || []).map((s, i) => `
          <div class="box" style="left:${s.x}%; top:${getY(s.cat)}px; width:${s.w||110}px; height:85px; border-color:${s.col||'rgba(255,255,255,0.1)'}; transform: translate(-50%, -50%);">
            ...
            <div style="color:${s.col||'#fff'}; font-size:${s.fs||1.4}em;"><span id="s-${i}-v" class="val">--</span><span id="s-${i}-u" class="uni"></span></div>
          </div>`).join('')}
...
class HealthDashboardCardEditor extends HTMLElement {
        ...
        ${['forme', 'sommeil', 'sante'].includes(this._tab) ? `
            ${(p.sensors || []).filter(s => s.cat === this._tab).map((s) => {
                ...
                <div class="sec">
                    ...
                    <div class="row"><input type="text" data-idx="${gi}" data-f="name" value="${s.name}"><input type="text" data-idx="${gi}" data-f="icon" value="${s.icon}"></div>
                    <label>Entité</label><input type="text" data-idx="${gi}" data-f="entity" value="${s.entity}">
                    <div class="row">
                        <div><label>Pos X (%)</label><input type="number" data-idx="${gi}" data-f="x" value="${s.x}"></div>
                        <div><label>Couleur</label><input type="color" data-idx="${gi}" data-f="col" value="${s.col||'#38bdf8'}"></div>
                    </div>
                    <div class="row">
                        <div><label>Taille Texte</label><input type="number" step="0.1" data-idx="${gi}" data-f="fs" value="${s.fs||1.4}"></div>
                        <div><label>Largeur</label><input type="number" data-idx="${gi}" data-f="w" value="${s.w||110}"></div>
                    </div>
                    <button class="del-s" data-idx="${gi}" style="width:100%; background:#422; color:#f52; border:1px solid #f52; padding:8px; margin-top:10px; border-radius:6px; cursor:pointer;">Supprimer</button>
                </div>`;
            }).join('')}
            <button style="width:100%; padding:12px; border-radius:8px; border:none; font-weight:bold; cursor:pointer; background:#38bdf8; color:#000;" id="add-s">+ AJOUTER À ${this._tab.toUpperCase()}</button>
        ` : ''}
      </div>
    `;
    this._attach();
  ...
  _attach() {
    ...
    this.querySelectorAll('input').forEach(el => {
      el.oninput = () => { // On utilise ONINPUT pour un enregistrement en direct
        const p = this._config[this._config.current_view];
        if(el.dataset.idx !== undefined) {
          p.sensors[el.dataset.idx][el.dataset.f] = el.value;
        } else if(el.id === 'ch') {
          this._config.card_height = el.value;
        } else {
...
window.customCards.push({ type: "health-dashboard-card", name: "Health Dashboard Smart V3.5.1", description: "Correction bug enregistrement." });

// HEALTH DASHBOARD CARD – VERSION 52 (AUTO-SYNC VIEW)
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .bg { position: absolute; inset: 0; background: url('${pData.image || ""}') center ${this._config.img_offset || 0}% / cover no-repeat; opacity: 0.4; transition: background 0.5s ease; }
      ...
      <div class="main-container">
        ...
        ${(pData.sensors || []).map((s, i) => `
            <div class="sensor" style="left:${s.x}%; top:${s.y}%; width:${s.name.toLowerCase().includes('corpulence') ? (this._config.imc_width||160) : (this._config.b_width||160)}px; height:${s.name.toLowerCase().includes('corpulence') ? (this._config.imc_height||69) : (this._config.b_height||69)}px;">
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        .tab-menu { display: flex; gap: 5px; margin-bottom: 15px; background: #333; padding: 5px; border-radius: 8px; }
        .t-btn { flex: 1; padding: 10px; border: none; background: transparent; color: #aaa; cursor: pointer; font-weight: bold; border-radius: 6px; }
        .t-btn.active { background: #38bdf8; color: white; }
      ...
      <div class="ed-box">
        <div class="tab-menu">
            <button class="t-btn ${pKey==='person1'?'active':''}" id="t-p1">RÉGLER PATRICK</button>
            <button class="t-btn ${pKey==='person2'?'active':''}" id="t-p2">RÉGLER SANDRA</button>
        </div>
        ...
        <div class="section">
            <h4 style="margin:0 0 10px 0; color:#38bdf8;">CAPTEURS DE ${p.name.toUpperCase()}</h4>
            ${p.sensors.map((s, i) => `
              <div class="s-card">
                <label>NOM</label><input type="text" class="s-inp" data-idx="${i}" data-f="name" value="${s.name}">
                <label>ICÔNE (ex: mdi:water)</label><input type="text" class="s-inp" data-idx="${i}" data-f="icon" value="${s.icon || ''}">
    ...
    // Switcher de vue ET d'onglet en même temps
...
window.customCards.push({ type: "health-dashboard-card", name: "Health Dashboard V52" });

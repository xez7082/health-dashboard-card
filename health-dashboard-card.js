...
class HealthDashboardCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    // Initialisation sécurisée des données par défaut si absentes
  ...
  updateSensors() {
    ...
    // 1. Mise à jour Poids et Réglette
    ...
    // 3. Mise à jour des Capteurs et Sparklines
    if (pData.sensors) {
...
class HealthDashboardCardEditor extends HTMLElement {
  ...
  render() {
    ...
    if (!this._config[pKey]) this._config[pKey] = { name: "Nouveau", sensors: [], start: 80, goal: 75, ideal: 70 };
    const p = this._config[pKey];

    this.innerHTML = `
      <style>
        .ed-box { padding: 12px; background: #1a1a1a; color: white; font-family: sans-serif; font-size: 13px; border-radius: 8px; }
      ...
      <div class="ed-box">
        ...
        <div class="section">
            ...
            <button style="width:100%; padding:10px; background:#4ade80; color:white; border:none; border-radius:4px; font-weight:bold; cursor:pointer;" id="add-sensor">➕ AJOUTER UN CAPTEUR</button>
        </div>
      </div>
    `;

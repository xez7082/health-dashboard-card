 ...
 * RETOUR DES ONGLES, SÉLECTEURS D'ENTITÉS ET PERSONNALISATION TOTALE
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  update() {
    ...
    (p.sensors || []).forEach((s, i) => setV(`s-${i}`, s.entity));
    ...
    if (stPoids) {
        ...
        if(range !== 0) {
            ...
            this.shadowRoot.getElementById('m-c').style.left = `${((this._num(p.start) - this._num(p.comfort)) / range) * 100}%`;
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .box { position: absolute; transform: translate(-50%, -50%); background: rgba(15, 23, 42, 0.85); display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 10; backdrop-filter: blur(8px); text-align: center; border-style: solid; }
...
class HealthDashboardCardEditor extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        .sec { background: #252525; padding: 10px; border-radius: 8px; margin-bottom: 10px; border-left: 3px solid #38bdf8; }
        label { color: #38bdf8; font-size: 10px; font-weight: bold; display: block; margin-top: 8px; text-transform: uppercase; }
        input, ha-entity-picker { width: 100%; margin-bottom: 5px; --paper-input-container-input-color: white; }
        ...
        <div id="form">
            ...
            ${this._tab === 'sante' ? `
                <div class="sec">
                    ...
                    <div class="grid">
                        <input type="number" data-f="imc_x" value="${p.imc_x}">
                        <input type="number" data-f="imc_y" value="${p.imc_y}">
                    </div>
                    <div class="grid">
                        <input type="number" data-f="imc_w" value="${p.imc_w}">
                        <input type="number" data-f="imc_h" value="${p.imc_h}">
                    </div>
                    <div class="grid">
                        <input type="number" data-f="imc_bw" value="${p.imc_bw}">
                ...
                <div class="sec">
                    ...
                    <div class="grid">
                        <input type="number" data-f="corp_bw" value="${p.corp_bw}">
                        <input type="text" data-f="corp_bc" value="${p.corp_bc}">
                    </div>
            ...
            ${this._tab === 'sensors' ? `
                ${(p.sensors || []).map((s, i) => `
                    <div class="sec">
                        ...
                        <button style="color:red; background:none; border:none; cursor:pointer;" onclick="this.getRootNode().host.delS(${i})">Supprimer ❌</button>
                    </div>
                `).join('')}
                <button id="add-s" style="width:100%; padding:10px; background:#444; color:white; border:none; cursor:pointer;">+ Ajouter Sensor</button>
            ` : ''}
            ...
            ${this._tab === 'design' ? `
                <div class="sec">
                    ...
                    <select id="ps" style="width:100%; padding:8px; background:#111; color:white;">
  ...
  _attach() {
    ...
    const add = this.querySelector('#add-s'); if(add) add.onclick = () => { p.sensors.push({name:'Nouveau', entity:'', x:50, y:50, w:100, h:70, bw:1, bc:'white', circle:false}); this._fire(); this.render(); };
  }

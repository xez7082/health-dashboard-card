/**
 * HEALTH DASHBOARD CARD – VERSION 67 (FIXED EDITOR)
 * Author: xez7082
 * License: MIT
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  renderSparkline(entityId) {
    return `<svg class="sparkline" viewBox="0 0 100 30" preserveAspectRatio="none" style="position:absolute; bottom:0; left:0; width:100%; height:30px; pointer-events:none; opacity:0.3;"><path d="M0,25 Q25,5 50,20 T100,10" fill="none" stroke="currentColor" stroke-width="2"/></svg>`;
  }
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .btn { border: 1px solid rgba(255,255,255,0.2); padding: 10px 20px; border-radius: 25px; background: rgba(0,0,0,0.5); color: white; cursor: pointer; font-size: 11px; font-weight: bold; backdrop-filter: blur(5px); }
        ...
        .prog-pointer { position: absolute; top: -12px; width: 4px; height: 30px; background: ${accentColor}; transition: left 1.2s ease-out; border-radius: 2px; }
        .prog-pointer::after { content: attr(data-val); position: absolute; top: -25px; left: 50%; transform: translateX(-50%); background: ${accentColor}; padding: 2px 8px; border-radius: 6px; font-size: 11px; white-space: nowrap; }
      ...
      <div class="main-container">
        ...
        ${(pData.sensors || []).map((s, i) => {
            const w = s.name.toLowerCase().includes('imc') ? (this._config.imc_width||160) : (this._config.b_width||160);
            const h = s.name.toLowerCase().includes('imc') ? (this._config.imc_height||75) : (this._config.b_height||75);
            return `<div class="sensor" style="left:${s.x}%; top:${s.y}%; width:${w}px; height:${h}px; color:${accentColor};">
              ${this.renderSparkline(s.entity)}
              <ha-icon icon="${s.icon || 'mdi:heart'}"></ha-icon>
              <div style="font-size:10px; color:#94a3b8;">${s.name}</div>
              <div id="value-${i}" style="font-weight:bold; font-size:14px; color:white;">--</div>
            </div>`;
        }).join('')}
      </div>
    `;
...
// CLASSE ÉDITEUR RÉTABLIE ET SÉCURISÉE
  ...
  render() {
    ...
    this.innerHTML = `
      <div style="padding: 20px; background: #2c3e50; color: white; border-radius: 8px;">
        <h3 style="color: #38bdf8; margin-bottom: 15px;">Configuration : ${pKey.toUpperCase()}</h3>
        
        <label style="display:block; margin-top:10px;">Couleur d'accentuation</label>
        <input type="color" id="accent-color" value="${p.accent_color || '#38bdf8'}" style="width:100%; height:40px; border:none; border-radius:4px; margin-bottom:10px;">

        <label style="display:block;">Nom de la personne</label>
        <input type="text" id="p-name" value="${p.name || ''}" style="width:100%; padding:8px; margin-bottom:10px; border-radius:4px; border:none;">

        <label style="display:block;">Image (URL)</label>
        <input type="text" id="p-image" value="${p.image || ''}" style="width:100%; padding:8px; margin-bottom:10px; border-radius:4px; border:none;">

        <div style="display:flex; gap:10px;">
          <div style="flex:1;">
            <label>Poids Départ (kg)</label>
            <input type="number" id="p-start" value="${p.start || 0}" style="width:100%; padding:8px; border-radius:4px; border:none;">
          </div>
          <div style="flex:1;">
            <label>Poids Idéal (kg)</label>
            <input type="number" id="p-ideal" value="${p.ideal || 0}" style="width:100%; padding:8px; border-radius:4px; border:none;">
          </div>
        </div>
        
        <p style="font-size: 11px; margin-top: 15px; color: #bdc3c7;">Note : Modifiez les capteurs directement dans le code YAML pour le moment.</p>
      </div>
    `;

    this.querySelector('#accent-color').addEventListener('change', (e) => this._updateConfig(pKey, 'accent_color', e.target.value));
    this.querySelector('#p-name').addEventListener('change', (e) => this._updateConfig(pKey, 'name', e.target.value));
    this.querySelector('#p-image').addEventListener('change', (e) => this._updateConfig(pKey, 'image', e.target.value));
    this.querySelector('#p-start').addEventListener('change', (e) => this._updateConfig(pKey, 'start', e.target.value));
    this.querySelector('#p-ideal').addEventListener('change', (e) => this._updateConfig(pKey, 'ideal', e.target.value));
  }

  _updateConfig(pKey, field, value) {
    const newConfig = JSON.parse(JSON.stringify(this._config));
    newConfig[pKey][field] = value;

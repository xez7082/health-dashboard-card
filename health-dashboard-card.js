  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        .del { position: absolute; top: 10px; right: 10px; background: #ff5252; color: white; padding: 4px 8px; font-size: 10px; }
      </style>
      <div class="ed">
        <div class="row" style="margin-bottom:15px;">
          <button id="sel-p1" style="flex:1; background:${v==='person1'?'#38bdf8':'#444'}">P1 (PATRICK)</button>
          <button id="sel-p2" style="flex:1; background:${v==='person2'?'#38bdf8':'#444'}">P2 (SANDRA)</button>
        </div>
        <div class="row">
          <button id="t-cfg" style="flex:1; background:${this._tab==='config'?'#38bdf8':'#444'}">⚙️ GÉNÉRAL & FOND</button>
          <button id="t-sns" style="flex:1; background:${this._tab==='sensors'?'#38bdf8':'#444'}">📊 CAPTEURS & TAILLES</button>
        </div>
        ...
        ` : `
          ...
          `).join('')}
          <button id="add-s" style="width:100%; background:#38bdf8; margin-top:10px;">+ AJOUTER UN CAPTEUR</button>
        `}
      </div>
    `;
    this._attach();

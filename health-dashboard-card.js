  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .prog-pointer { position: absolute; top: -14px; width: 4px; height: 36px; background: ${accentColor}; transition: left 1s ease-in-out; border-radius: 2px; }
        .prog-pointer::after { content: attr(data-val); position: absolute; top: -28px; left: 50%; transform: translateX(-50%); background: ${accentColor}; padding: 3px 10px; border-radius: 8px; font-size: 12px; font-weight: bold; white-space: nowrap; color: #000; }
        ...
        .sensor { position: absolute; transform: translate(-50%, -50%); border-radius: 16px; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255,255,255,0.15); display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 10; padding: 10px; backdrop-filter: blur(12px); }

 ...
 * RÉTABLISSEMENT : AJOUT DE CAPTEURS + CHOIX DES ENTITÉS + STYLE COMPLET
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  updateSensors() {
    ...
    // Mise à jour des valeurs (IMC, Corpulence et Libres)
    const setV = (id, ent) => {
    ...
    if (stPoids) {
        ...
        if(ptr) ptr.style.left = `${range !== 0 ? Math.max(0, Math.min(100, ((this._num(pData.start) - actuel) / range) * 100)) : 0}%`;
    }
  }
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .bg { position: absolute; inset: 0; background: url('${pData.image}') center/cover; opacity: 0.4; }
        ...
        .rule { position: absolute; bottom: 40px; left: 5%; width: 90%; height: 8px; background: rgba(255,255,255,0.1); border-radius: 4px; }
        .fill { position: absolute; inset: 0; background: linear-gradient(90deg, #f87171, #fbbf24, #4ade80); border-radius: 4px; opacity: 0.7; }
        .ptr { position: absolute; top: -14px; width: 3px; height: 36px; background: white; transition: left 1s; }
        .bub { position: absolute; top: -42px; left: 50%; transform: translateX(-50%); background: white; color: black; padding: 4px 10px; border-radius: 8px; font-weight: bold; border: 2px solid ${accent}; }
      </style>
      ...
      <div class="main">
        ...
        <div class="box" style="left:${pData.imc_x}%; top:${pData.imc_y}%; width:${pData.imc_w}px; height:${pData.imc_circle ? pData.imc_w : pData.imc_h}px; border-radius:${pData.imc_circle ? '50%' : round + 'px'}; border: ${pData.imc_b_w}px solid ${pData.imc_b_c};">
            ...
            <div style="font-size:10px;">${pData.imc_name}</div>
            <div id="imc-val" style="font-weight:bold;">--</div>
        </div>
        ...
        <div class="box" style="left:${pData.corp_x}%; top:${pData.corp_y}%; width:${pData.corp_w}px; height:${pData.corp_circle ? pData.corp_w : pData.corp_h}px; border-radius:${pData.corp_circle ? '50%' : round + 'px'}; border: ${pData.corp_b_w}px solid ${pData.corp_b_c};">
            ...
            <div style="font-size:10px;">${pData.corp_name}</div>
            <div id="corp-val" style="font-weight:bold;">--</div>
        </div>
        ...
        <div class="rule"><div class="fill"></div><div id="progression-pointer" class="ptr"><div id="pointer-label" class="bub">--</div></div></div>
      </div>
    `;
  }
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        .tab { padding: 10px; cursor: pointer; background: #333; border: none; color: #888; flex: 1; font-size: 11px; }
        ...
        .del { color: #f87171; font-size: 10px; cursor: pointer; margin-top: 5px; display: block; text-align: right; }
      </style>
      <div class="ed">
        <div class="tabs">
        ...
        ${this._tab === 'health' ? `
            <div class="sec">
                <label>IMC (Entité / Nom / Icône)</label>
                <input type="text" id="imce" value="${p.imc_entity}">
                <div class="grid"><input type="text" id="imcn" value="${p.imc_name}"> <input type="text" id="imci" value="${p.imc_icon}"></div>
                <label>Position X/Y % et Taille W/H</label>
        ...
        ${this._tab === 'sensors' ? `
            ${(p.sensors || []).map((s, i) => `
                <div class="sec">
                    <label>Capteur ${i+1} : Entité</label>
                    <input type="text" class="s-ent" data-idx="${i}" value="${s.entity}">
                    <div class="grid">
                        <div><label>Nom</label><input type="text" class="s-nom" data-idx="${i}" value="${s.name}"></div>
                        <div><label>Icône</label><input type="text" class="s-ico" data-idx="${i}" value="${s.icon}"></div>
                    </div>
                    <div class="grid">
                    ...
                    <div class="grid">
                        <div><label>Largeur</label><input type="number" class="s-w" data-idx="${i}" value="${s.w}"></div>
                        <div><label>Hauteur</label><input type="number" class="s-h" data-idx="${i}" value="${s.h}"></div>
                    </div>
                    <div class="grid">
                        <div><label>Bordure px</label><input type="number" class="s-bw" data-idx="${i}" value="${s.b_w || 0}"></div>
                        <div style="display:flex; align-items:center; gap:5px; margin-top:15px;"><input type="checkbox" class="s-c" data-idx="${i}" ${s.circle?'checked':''}> <span style="font-size:10px;">ROND</span></div>
                    </div>
                    <span class="del" data-idx="${i}">Supprimer ❌</span>
                </div>
            `).join('')}
    ...
    this.querySelectorAll('.s-nom').forEach(el => el.oninput = (e) => { p.sensors[el.dataset.idx].name = e.target.value; fire(); });
    this.querySelectorAll('.s-ico').forEach(el => el.oninput = (e) => { p.sensors[el.dataset.idx].icon = e.target.value; fire(); });
    ...
    this.querySelectorAll('.del').forEach(el => el.onclick = () => { p.sensors.splice(el.dataset.idx, 1); fire(); this.render(); });
    
    const btnAdd = this.querySelector('#add-s');
    if(btnAdd) btnAdd.onclick = () => { 
        if(!p.sensors) p.sensors = []; 
        p.sensors.push({entity:"", name:"Nouveau", icon:"mdi:heart", x:50, y:50, w:100, h:70, b_w:1, b_c:"rgba(255,255,255,0.2)", circle:false}); 
        fire(); this.render(); 
    };

/**
 * HEALTH DASHBOARD CARD – V2.1.1
 * Correction du bouton de suppression des capteurs dans l'éditeur.
 */
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="main-container">
        ...
        ${pData.imc_entity ? `<div class="sensor-card" style="left:${pData.imc_x}%; top:${pData.imc_y}%; width:${pData.imc_w}px; height:${pData.imc_h}px;"><ha-icon icon="${pData.imc_icon || 'mdi:scale-bathroom'}"></ha-icon><div style="font-size:10px; opacity:0.8;">${pData.imc_name}</div><div id="imc-val" style="font-weight:900;">--</div></div>` : ''}
        ${pData.corp_entity ? `<div class="sensor-card" style="left:${pData.corp_x}%; top:${pData.corp_y}%; width:${pData.corp_w}px; height:${pData.corp_h}px;"><ha-icon icon="${pData.corp_icon || 'mdi:human-male'}"></ha-icon><div style="font-size:10px; opacity:0.8;">${pData.corp_name}</div><div id="corp-val" style="font-weight:900;">--</div></div>` : ''}
...
class HealthDashboardCardEditor extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        .del-btn { width: 100%; margin-top: 10px; background: #f87171; border: none; color: white; padding: 8px; border-radius: 4px; cursor: pointer; font-weight: bold; }
      </style>
      <div class="ed-box">
        ...
        <div class="section">
            ...
            ${this._activeTab === 'health' ? `
                <div class="sub-sec"><label>IMC</label><input type="text" id="inp-imce" value="${p.imc_entity || ''}"><div class="grid"><div><label>L</label><input type="number" id="inp-imcw" value="${p.imc_w}"></div><div><label>H</label><input type="number" id="inp-imch" value="${p.imc_h}"></div><div><label>X</label><input type="number" id="inp-imcx" value="${p.imc_x}"></div><div><label>Y</label><input type="number" id="inp-imcy" value="${p.imc_y}"></div></div></div>
                <div class="sub-sec"><label>CORPULENCE</label><input type="text" id="inp-corpe" value="${p.corp_entity || ''}"><div class="grid"><div><label>L</label><input type="number" id="inp-corpw" value="${p.corp_w}"></div><div><label>H</label><input type="number" id="inp-corph" value="${p.corp_h}"></div><div><label>X</label><input type="number" id="inp-corpx" value="${p.corp_x}"></div><div><label>Y</label><input type="number" id="inp-corpy" value="${p.corp_y}"></div></div></div>
            ` : ''}
  ...
  _attachEvents(pKey) {
    ...
    if(this._activeTab === 'profile') { bind('#inp-name', 'name'); bind('#inp-img', 'image'); bind('#inp-start', 'start'); bind('#inp-goal', 'goal'); bind('#inp-ideal', 'ideal'); }
    if(this._activeTab === 'health') { bind('#inp-imce', 'imc_entity'); bind('#inp-imcw', 'imc_w'); bind('#inp-imch', 'imc_h'); bind('#inp-imcx', 'imc_x'); bind('#inp-imcy', 'imc_y'); bind('#inp-corpe', 'corp_entity'); bind('#inp-corpw', 'corp_w'); bind('#inp-corph', 'corp_h'); bind('#inp-corpx', 'corp_x'); bind('#inp-corpy', 'corp_y'); }
    ...
    if(this._activeTab === 'sensors') {
        ...
        // CORRECTION ICI : Écouteur sur chaque bouton supprimer
        this.querySelectorAll('.del-btn').forEach(btn => {
            btn.onclick = () => {
                const idx = parseInt(btn.dataset.idx);
                this._config[pKey].sensors.splice(idx, 1);
                this._fire();
                this.render(); // On re-rend l'éditeur pour mettre à jour la liste
            };
        });
...
window.customCards.push({ type: "health-dashboard-card", name: "Health Dashboard V2.1.1" });

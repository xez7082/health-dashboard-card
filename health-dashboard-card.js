/**
 * HEALTH DASHBOARD CARD – V4.1.0
 * CUSTOMISATION TOTALE : TAILLES, POLICES, POSITIONS X/Y
 */
...
class HealthDashboardCard extends HTMLElement {
  ...
  render() {
    ...
    const getBaseY = (cat) => (cat === 'sommeil' ? 220 : (cat === 'sante' ? 360 : 100));
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .box { position: absolute; background: rgba(30, 41, 59, 0.7); display: flex; flex-direction: column; align-items: center; justify-content: center; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(5px); z-index: 10; transform: translate(-50%, -50%); overflow: hidden; }
        .lbl { text-transform: uppercase; text-align: center; opacity: 0.7; line-height: 1; }
        .val { font-weight: 800; margin-top: 2px; color: #fff; }
        
        .weight-card { position: absolute; bottom: 12px; left: 12px; right: 12px; height: 115px; background: rgba(15, 23, 42, 0.9); border-radius: 18px; padding: 12px; border: 1px solid rgba(56, 189, 248, 0.2); box-sizing: border-box; z-index: 20; }
      ...
      <div class="main">
        ...
        ${(p.sensors || []).map((s, i) => {
          const yPos = s.y_off ? (getBaseY(s.cat) + parseInt(s.y_off)) : getBaseY(s.cat);
          return `
        ...
        <div class="weight-card">
          <div style="display:flex; justify-content:space-between; align-items:center;">
            <div><div style="font-size: 8px; opacity: 0.5;">ACTUEL</div><div style="font-size: 22px; font-weight:900; color:#38bdf8;" id="weight-val">--</div></div>
            <div style="text-align:right;"><div id="diff-val" style="font-size:14px; font-weight:bold;">--</div><div style="font-size: 8px; opacity: 0.5;">DEPUIS DÉPART</div></div>
          </div>
          ...
          <div style="display:flex; justify-content:space-between; font-size:9px; font-weight:bold;">
            <span>${p.start}kg</span><span id="reste-val" style="color:#38bdf8;">--</span><span>${p.ideal}kg</span>
          </div>
        </div>
      </div>
    `;
...
class HealthDashboardCardEditor extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        .ed { padding: 12px; background: #1c1c1e; color: white; font-family: sans-serif; font-size: 12px; }
        .tabs { display: flex; gap: 5px; margin-bottom: 15px; }
        .tab { flex: 1; padding: 8px; background: #2c2c2e; color: #fff; border: none; border-radius: 4px; cursor: pointer; }
        .tab.active { background: #38bdf8; color: #000; }
        .s-card { background: #2c2c2e; padding: 10px; border-radius: 8px; margin-bottom: 10px; border-left: 3px solid #38bdf8; }
        .row { display: flex; gap: 8px; margin-top: 5px; }
        .row div { flex: 1; }
        input, select { width: 100%; padding: 6px; background: #000; color: #fff; border: 1px solid #444; border-radius: 4px; box-sizing: border-box; font-size: 11px; }
        label { font-size: 10px; opacity: 0.6; display: block; margin-top: 5px; }
        .btn-add { width: 100%; padding: 10px; background: #38bdf8; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; margin-top: 10px; }
      </style>
      <div class="ed">
        <div class="tabs">
          <button class="tab ${this._tab==='config'?'active':''}" id="t-cfg">CONFIG</button>
        ...
        ` : `
          ${(p.sensors || []).map((s, i) => `
            <div class="s-card">
              <div style="display:flex; justify-content:space-between"><strong>${s.name || 'Capteur'}</strong><button style="background:red; color:white; border:none; padding:2px 5px; cursor:pointer" data-idx="${i}" class="del">X</button></div>
              <div class="row">
                <div><label>Nom</label><input type="text" data-idx="${i}" data-f="name" value="${s.name}"></div>
                <div><label>Entité</label><input type="text" data-idx="${i}" data-f="entity" value="${s.entity}"></div>
              </div>
              <div class="row">
                <div><label>X (%)</label><input type="number" data-idx="${i}" data-f="x" value="${s.x}"></div>
                <div><label>Ajust. Y (px)</label><input type="number" data-idx="${i}" data-f="y_off" value="${s.y_off || 0}"></div>
              ...
              <div class="row">
                <div><label>Largeur (px)</label><input type="number" data-idx="${i}" data-f="w" value="${s.w || 85}"></div>
                <div><label>Hauteur (px)</label><input type="number" data-idx="${i}" data-f="h" value="${s.h || 55}"></div>
              </div>
              <div class="row">
                <div><label>Police Titre (px)</label><input type="number" data-idx="${i}" data-f="fs_l" value="${s.fs_l || 7}"></div>
                <div><label>Police Val. (px)</label><input type="number" data-idx="${i}" data-f="fs_v" value="${s.fs_v || 12}"></div>
              </div>
            </div>
          `).join('')}
  ...
  _attach() {
    ...
    if(addBtn) addBtn.onclick = () => {
      this._config[this._config.current_view].sensors.push({name:'Nouveau', entity:'', cat:'forme', x:50, w:85, h:55, fs_l:7, fs_v:12});
...
window.customCards.push({ type: "health-dashboard-card", name: "Health Dashboard Smart V4.1.0", description: "Customisation Totale" });

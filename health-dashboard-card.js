// HEALTH DASHBOARD CARD – VERSION 35 (STABLE & ANTI-CRASH)
  ...
  render() {
    ...
    this.innerHTML = `
      ...
      <div class="ed-wrap">
        ...
        <div class="section">
          <h4>⚖️ JAUGE IMC</h4>
          <label>Position Titre : ${this._config.imc_title_pos}px</label>
        ...
        <div id="list">
          ${person.sensors.map((s, i) => `
            <div class="s-item">
              ...
              <button class="del" data-idx="${i}" style="width:100%; background:#7f1d1d; color:white; border:none; padding:4px; margin-top:5px; border-radius:4px;">SUPPRIMER</button>
            </div>
          `).join('')}
        </div>
        <button id="add" style="width:100%; margin-top:10px; padding:10px; background:#065f46; color:white; border:none; border-radius:6px;">+ AJOUTER</button>
      </div>`;
    this._setup();
  }
  _setup() {
...
window.customCards.push({ type: "health-dashboard-card", name: "Health Dashboard V35" });

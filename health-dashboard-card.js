// HEALTH DASHBOARD CARD – VERSION AVEC PREVIEW ET POSITIONNEMENT COMPLET
  ...
  render() {
    ...
    this.innerHTML = `
      <style>
        ...
        /* PREVIEW DE L'IMAGE AVEC CAPTEURS */
        .preview-section {
          background: #f9f9f9;
          border-radius: 8px;
          padding: 16px;
      ...
      <div class="editor">
        <div class="info-box">
          💡 <strong>Astuce :</strong> Utilisez la prévisualisation ci-dessous pour voir le positionnement des capteurs sur l'image. Ajustez les valeurs X et Y pour positionner précisément.
        </div>

        <div class="tabs">
          <button class="tab-btn ${this.currentTab === "person1" ? "active" : ""}" data-tab="person1">
            👤 ${this._config.person1.name}
          </button>
          <button class="tab-btn ${this.currentTab === "person2" ? "active" : ""}" data-tab="person2">
            👤 ${this._config.person2.name}
          </button>
        </div>
        ...
        <div class="section">
          ...
          <div class="field">
            ...
            <input 
              ...
              class="input-name" 
              value="${person.name || ""}"
              placeholder="Ex: Jean, Marie, Sandra"
            />
          </div>
  ...
  renderSensors(sensors) {
    ...
    return sensors.map((sensor, index) => `
      <div class="sensor-item" data-index="${index}">
        ...
        <div class="field">
          <label>🎨 Couleur de la case</label>
          <div class="color-picker-wrapper">
            <input 
              type="color" 
              class="sensor-color color-picker" 
              data-index="${index}"
              value="${sensor.color || '#2196f3'}"
            />
            <span class="color-value">${sensor.color || '#2196f3'}</span>
          </div>
        </div>

        <div class="position-fields">
          <label style="display: block; margin-bottom: 8px;">📍 Position sur l'image</label>
          <div class="field-row">
            <div class="field" style="margin: 0;">
              ...
              <input 
                ...
                step="1"
              />
              <small>Gauche: 5 • Centre: 50 • Droite: 95</small>
            </div>
            <div class="field" style="margin: 0;">
              ...
              <input 
                ...
                step="1"
              />
              <small>Haut: 5 • Centre: 50 • Bas: 95</small>
            </div>
          </div>
        </div>
      </div>
  ...
  attachListeners() {
    ...
    if (genderSelect) {
      genderSelect.onchange = (e) => {
        ...
        this.configChanged();
        this.render(); // Re-render pour mettre à jour l'image par défaut
      };
    }
    ...
    if (imageInput) {
      imageInput.oninput = (e) => {
        ...
        this.configChanged();
        this.render(); // Re-render pour mettre à jour la preview
      };
    }
    ...
    const colorInputs = this.querySelectorAll(".sensor-color");
    const xInputs = this.querySelectorAll(".sensor-x");
    const yInputs = this.querySelectorAll(".sensor-y");
    ...
    colorInputs.forEach(input => {
      input.oninput = (e) => {
        ...
        this._config[this.currentTab].sensors[index].color = color;
    ...
    xInputs.forEach(input => {
      input.oninput = (e) => {
        const index = parseInt(e.target.dataset.index);
        this._config[this.currentTab].sensors[index].x = parseFloat(e.target.value);
        this.configChanged();
        this.render(); // Re-render pour mettre à jour la preview
      };
    });
    ...
    yInputs.forEach(input => {
      input.oninput = (e) => {
        const index = parseInt(e.target.dataset.index);
        this._config[this.currentTab].sensors[index].y = parseFloat(e.target.value);
        this.configChanged();
        this.render(); // Re-render pour mettre à jour la preview
      };
    });
...
window.customCards.push({
  ...
  description: "Carte de suivi de santé avec positionnement manuel et prévisualisation",
  preview: true,
});

console.info(
  "%c HEALTH-DASHBOARD-CARD %c v3.1.0 ",
